    ...
    for (const categoryPointLocation in props.categoryMap.Category) {
        cards.push(
            <CategoryMapLocation 
                ...
                categoryPointLocation={props.categoryMap.Category[categoryPointLocation]}/>
        );
    }
    ...
    return (
        <Card className="toolbox">
            ...
            <Container className="scroller">
                ...
                <Button
                    className="inner"
                    onClick={() => props.handler.createTrans('Category', {
                        "Name" : "",
                        "Polygons": [],
                        "Color" : "#FF0000",
                    })}>
        ...
        <Card className="inner">
            <Card.Body style={{backgroundColor: "#141488", color: "white", height: "40px", padding: "5px"}}>
                <input 
                    ...
                    onChange={(val) => props.handler.updateTrans(`Category[${props.index}].Name`, val.target.value)}/>
                <BsXLg 
                    className="invisibleButton"
                    onClick={(val) => props.handler.deleteTrans('Category', props.categoryPointLocation)}/>
            </Card.Body>
            <Container style={{padding: "20px"}}>
                ...
                <Container>
                    ...
                    <Button
                        className="inner"
                        onClick={() => props.handler.createTrans(`Category[${props.index}].Polygons`, {
                            "Coordinates" : [],
                        })}>
                        Add new
                    </Button>
                ...
                <ColorWidget
                    ...
                    onChange={(val) => props.handler.updateTrans(`Category[${props.index}].Color`, val)}/>
...
function CategoryMapRegion(props) {
    return (
        <Card className="inner">
            <Card.Body style={{border: "1px solid black", borderRadius: "5px", color: "black", height: "40px", padding: "5px"}}>
                Region {props.index}
                <BsXLg 
                    className="invisibleButton"
                    style={{position: "absolute", right: "5px", top: "12px"}}
                    onClick={(val) => props.handler.deleteTrans(`Category[${props.parentIndex}].Polygons`, props.region)}/>
            </Card.Body>
        </Card>
    )
}